...
long long Tree::GetMax(size_t node, std::pair<int, int> start_finish, int left,
                       int right, std::pair<int, int> borders) {
  ...
  if (start == left && finish == right) {
    long long index_start = std::lower_bound(tree_[node].begin(),
                                             tree_[node].end(), borders.first) -
                            tree_[node].begin();
...
void Tree::NewTree(size_t node, int start, int finish) {
  if (start == finish) {
